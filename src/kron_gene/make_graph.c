...
static void compute_edge_range(int rank, int size, int64_t M, int64_t* start_idx, int64_t* end_idx) {
  ...
  *start_idx = (rankc + 0) * (M / sizec) + (rankc + 0 < (M % sizec) ? rankc + 0 : (M % sizec));
  *  end_idx = (rankc + 1) * (M / sizec) + (rankc + 1 < (M % sizec) ? rankc + 1 : (M % sizec));
}

void make_graph(int log_numverts, int64_t M,
		uint64_t userseed1, uint64_t userseed2,
		int64_t* nedges_ptr, packed_edge** result_ptr) {
  int rank, size;
...
void make_graph(int log_numverts, int64_t M,
		uint64_t userseed1, uint64_t userseed2,
		int64_t* nedges_ptr_in, packed_edge **result_ptr_in) {
  /* not used. */
  log_numverts = log_numverts;
  ...
  *result_ptr = edges;
  ...
  /* In OpenMP and XMT versions, the inner loop in generate_kronecker_range is parallel. */
