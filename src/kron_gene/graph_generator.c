...
/* R-MAT */
/* #define INITIATOR_A_NUMERATOR  4500 */
/* #define INITIATOR_B_NUMERATOR  2500 */
/* #define INITIATOR_C_NUMERATOR  1500 */
/* #define INITIATOR_DENOMINATOR 10000 */
...
#endif
  unsigned int adjusted_bc_numerator = INITIATOR_BC_NUMERATOR + spk_noise_factor;
...
/* static */
static inline void make_one_edge(int64_t nverts, int level, int lgN,
				 mrg_state *st, packed_edge *result,
...
int64_t generate_kronecker_edges(int verbose, const uint_fast32_t seed[5], int logN,
  ...
  for (ei = start_edge; ei < end_edge; ++ei) {
    ...
    if ( verbose ) {
      int t = tm_interval(&tm,
			  1000.0 /* interval in msecs */,
			  0.0    /* time limit in msecs (0.0: unlimited) */);
      if ( ei == end_edge-1 || t ) {
	print_prbar((double)ei/end_edge, "\t", '#', (ei == end_edge-1) ? '\n' : '\r');
	/* printf("[%5.1f] %.2f%%\n", tm_elapsed(&tm) * 1e-3, 1e2*ei/(end_edge-start_edge)); */
      }
    }
  }
