...
#include "dump.h"
//#include "external_full_construction.h"
...
struct graph_t *graph_construction(struct edgelist_t *list) {
  ...
  /* external full graph construction  */
  struct dumpfiles_t *DF_E = NULL;
  DF_E = init_dumpfile_info_edgelist("", 0);
  printf ("[ external full graph construction ]\n");
  t1 = get_seconds();
  I64_t *num_edges = external_full_graph_construction(list, DF_E);
  t2 = get_seconds();
  printf ("external full graph construction takes %.3lf seconds", t2-t1);
...
#if 1
  for (int k = 0; k < DF_G_s->num_files; k++) {
...
#if 0
  printf("dump onmem-graph ...");
  t1 = get_seconds();

  struct dumpfiles_t *DF_G_s_onm = NULL;
  struct dumpfiles_t *DF_G_e_onm = NULL;

  DF_G_s_onm = init_dumpfile_info_graph("onm", 1);
  DF_G_e_onm = init_dumpfile_info_graph("onm", 0);
  fopen_files_new(DF_G_s_onm);
  fopen_files_new(DF_G_e_onm);

  dump_subgraph_fwrite(G->BG_list, G->num_graphs, DF_G_s_onm, DF_G_e_onm);

  fclose_files(DF_G_s_onm);
  fclose_files(DF_G_e_onm);

  free_files(DF_G_s_onm);
  free_files(DF_G_e_onm);

  t2 = get_seconds();
  printf("done. takes %.3f seconds\n", t2-t1);
#endif
